...
************************************************************/
def pushDockerImages(String DOCKER_USER, String DOCKER_APP_NAME, String DOCKER_TAG)
{
   try {
      wrap([$class: 'AnsiColorBuildWrapper']) {
	   ...
	   withCredentials([string(credentialsId: 'Docker-Pass', variable: 'DockerHubPass')]) {
           sh "docker login -u ${DOCKER_USER} -p ${DockerHubPass}" && "docker push ${DOCKER_USER}/${DOCKER_APP_NAME}:${DOCKER_TAG}"
         }
      }
   }
   catch (Exception caughtException) {
