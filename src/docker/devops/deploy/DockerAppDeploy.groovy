...
def UnDeployContainer(String DEPLOYMENT_SERVERS, String LINUX_USER, String CONTAINER_NAME, String LINUX_CREDENTIALS) 
{
   try {
...
def DeployContainer(String DEPLOYMENT_SERVERS, String LINUX_USER, String DOCKER_USER, String DOCKER_APP_NAME, String DOCKER_TAG, String CONTAINER_NAME, String LINUX_CREDENTIALS)
{
   try {
      wrap([$class: 'AnsiColorBuildWrapper']) {
        ...
        for (LINUX_SERVER in DEPLOYMENT_SERVERS.split(',')) {
        sshagent(["${LINUX_CREDENTIALS}"]) {
        sh "ssh -t -o StrictHostKeyChecking=no ${LINUX_USER}@${LINUX_SERVER} && docker run -p 8080:8080 -d --name ${CONTAINER_NAME} ${DOCKER_USER}/${DOCKER_APP_NAME}:${DOCKER_TAG}"
         }
       }
     }
   }
